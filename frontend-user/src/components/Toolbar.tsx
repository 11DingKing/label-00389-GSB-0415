...
const tools: { type: ToolType; label: string; icon: JSX.Element }[] = [
  { type: 'select', label: '选择', icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.75} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M15.042 21.672L13.684 16.6m0 0l-2.51 2.225.569-9.47 5.227 7.917-3.286-.672zM12 2.25V4.5m5.834.166l-1.591 1.591M20.25 10.5H18M7.757 14.743l-1.59 1.59M6 10.5H3.75m4.007-4.243l-1.59-1.59" /></svg> },
  { type: 'pan', label: '移动', icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.75} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M3.75 3.75v4.5m0-4.5h4.5m-4.5 0L9 9M3.75 20.25v-4.5m0 4.5h4.5m-4.5 0L9 15M20.25 3.75h-4.5m4.5 0v4.5m0-4.5L15 9m5.25 11.25h-4.5m4.5 0v-4.5m0 4.5L15 15" /></svg> },
  { type: 'pen', label: '画笔', icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.75} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L6.832 19.82a4.5 4.5 0 01-1.897 1.13l-2.685.8.8-2.685a4.5 4.5 0 011.13-1.897L16.863 4.487zm0 0L19.5 7.125" /></svg> },
  { type: 'highlighter', label: '荧光笔', icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.75} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M9.53 16.122a3 3 0 00-5.78 1.128 2.25 2.25 0 01-2.4 2.245 4.5 4.5 0 008.4-2.245c0-.399-.078-.78-.22-1.128zm0 0a15.998 15.998 0 003.388-1.62m-5.043-.025a15.994 15.994 0 011.622-3.395m3.42 3.42a15.995 15.995 0 004.764-4.648l3.876-5.814a1.151 1.151 0 00-1.597-1.597L14.146 6.32a15.996 15.996 0 00-4.649 4.763m3.42 3.42a6.776 6.776 0 00-3.42-3.42" /></svg> },
  { type: 'text', label: '文字', icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.75} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M7 8h10M7 12h4m1 8l-4-4H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-3l-4 4z" /></svg> },
  ...
  { type: 'polygon', label: '多边形', icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={1.75} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M12 2l4.5 9 4.5 3-4.5 3-4.5 9-4.5-9-4.5-3 4.5-3z" /></svg> },
  ...
  return (
    <div className="bg-white border-t sm:border-t-0 sm:border-b border-gray-100 px-2 sm:px-4 py-1.5 sm:py-2.5 relative z-30">
      ...
      {showColors && colorBtnRef.current && (
        <>
          ...
          <div 
            ...
            style={{
              left: colorBtnRef.current.getBoundingClientRect().left + colorBtnRef.current.offsetWidth / 2,
              bottom: window.innerHeight - colorBtnRef.current.getBoundingClientRect().top + 8,
          ...
          >
            ...
            <div className="grid grid-cols-4 gap-2">
              {colors.map((c) => (
                <button
                  key={c.value}
                  onClick={() => { onColorChange(c.value); setShowColors(false); showToast(`${c.name}色`, 'info'); }}
      ...
      <div className="flex items-center gap-1 sm:gap-2">
        ...
        <div className="flex-1 overflow-x-auto scrollbar-hide -mx-1 px-1">
          <div className="flex items-center gap-1 sm:gap-0.5 sm:bg-gray-50 sm:rounded-xl sm:p-1 w-max">
            {tools.map((tool) => (
              ...
              >
                {tool.icon}
                <span className="text-[9px] sm:hidden leading-none">{tool.label.slice(0, 2)}</span>
              </button>
            ))}
          </div>
        </div>
        ...
        >
          <div className="w-6 h-6 sm:w-5 sm:h-5 rounded-md shadow-sm border border-gray-200" style={{ backgroundColor: currentColor }} />
          <span className="text-[9px] sm:hidden text-gray-500">颜色</span>
          <svg className="hidden sm:block w-3.5 h-3.5 text-gray-400" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
        ...
        <div className="hidden sm:flex items-center gap-0.5">
          <button 
            onClick={() => { if (canUndo) { onUndo(); showToast('撤销', 'info'); } }} 
            disabled={!canUndo}
            title="撤销" 
          ...
          <button 
            onClick={() => { if (canRedo) { onRedo(); showToast('重做', 'info'); } }} 
            disabled={!canRedo}
            title="重做" 
          ...
          <button onClick={() => { onClear(); showToast('已清除', 'warning'); }} title="清除" className="p-2 rounded-lg text-gray-500 hover:text-red-500 hover:bg-red-50 transition-colors">
            ...
            <div className="absolute bottom-full right-2 mb-2 p-3 bg-white rounded-2xl shadow-xl border border-gray-100 z-50 animate-scale-in min-w-[220px]">
              ...
              <div className="mb-3 pb-3 border-b border-gray-100">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-xs font-medium text-gray-600">{currentLineWidth}px</span>
                </div>
                <input
                  type="range"
                  min="1"
              ...
              <div className="grid grid-cols-3 gap-2">
                <button 
                  onClick={() => { if (canUndo) { onUndo(); showToast('撤销', 'info'); setShowMore(false); } }} 
                  disabled={!canUndo}
                ...
                <button 
                  onClick={() => { if (canRedo) { onRedo(); showToast('重做', 'info'); setShowMore(false); } }} 
                  disabled={!canRedo}
                ...
                <button 
                  onClick={() => { onClear(); showToast('已清除', 'warning'); setShowMore(false); }} 
