...
interface AnnotationCanvasProps {
  ...
  onAddPolygonVertex: (point: Point) => void;
  onClosePolygon: () => void;
...
export default function AnnotationCanvas({
  ...
  const redraw = useCallback(() => {
    ...
    if (isPolygonDrawing && currentPolygonPoints.length > 0) {
      const scaledPoints = currentPolygonPoints.map((p) =>
        baseToScreen(p, scale),
      );
      const scaledLineWidth = currentLineWidth * scale;
