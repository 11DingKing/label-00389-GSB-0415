...
interface AnnotationCanvasProps {
  ...
  onContinueDrawing: (point: Point, bounds?: { width: number; height: number }) => void;
  onEndDrawing: () => void;
  onAddText: (point: Point, text: string) => void;
  onDeleteSelected: () => void;
  findAnnotationAtPoint: (point: Point, annotations: Annotation[]) => string | null;
...
// 绘制标注（使用屏幕坐标）
function drawAnnotation(ctx: CanvasRenderingContext2D, annotation: Annotation, scale: number): void {
  ...
  switch (annotation.type) {
    ...
    case 'rectangle':
      if (scaledPoints.length >= 2) {
        const [start, end] = [scaledPoints[0], scaledPoints[scaledPoints.length - 1]];
...
function drawArrow(ctx: CanvasRenderingContext2D, start: Point, end: Point, lineWidth: number): void {
...
export default function AnnotationCanvas({
  width, height, scale, annotations, currentTool, currentColor, currentLineWidth,
  isDrawing, isDragging, selectedId, getCurrentAnnotation, onStartDrawing, 
  onContinueDrawing, onEndDrawing, onAddText, onDeleteSelected, findAnnotationAtPoint,
  onRequestTextInput,
  ...
  const redraw = useCallback(() => {
    ...
    annotations.forEach((a) => {
      const isHoveredForErase = currentTool === 'eraser' && hoveredAnnotation === a.id;
      ...
      if (selectedId === a.id && !isHoveredForErase) {
        ...
        if (bounds) {
          ...
          ctx.setLineDash([]);
          // 角点大小根据边界框大小调整
          const cornerSize = Math.min(3, Math.min(bounds.width, bounds.height) / 8);
          const corners = [
          ...
          corners.forEach(c => { ctx.beginPath(); ctx.arc(c.x, c.y, cornerSize, 0, Math.PI * 2); ctx.fill(); });
          ctx.restore();
        }
      }
    });
  ...
  }, [annotations, getCurrentAnnotation, isDrawing, currentTool, eraserPos, hoveredAnnotation, selectedId, getAnnotationBounds, scale]);

  useEffect(() => {
    const animate = () => { redraw(); animationFrameRef.current = requestAnimationFrame(animate); };
    animate();
  ...
  const handlePointerDown = useCallback((e: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    if (currentTool === 'text') {
      ...
      onStartDrawing(basePoint);
    }
  }, [currentTool, scale, onStartDrawing, textInput, textValue, onAddText, showToast, onRequestTextInput]);
  ...
  }, [isDrawing, isDragging, scale, width, height, onContinueDrawing, currentTool, findAnnotationAtPoint, annotations]);
  ...
  return (
    <div className="relative" style={{ width, height }}>
      ...
      {/* 移动端：底部抽屉 */}
      {textInput && (
        <div className="md:hidden fixed inset-0 z-50 bg-black/50 animate-fade-in" onClick={handleTextCancel}>
          <div 
          ...
          >
            ...
            <div className="px-4 pb-4 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <span className="w-5 h-5 rounded-full border-2 border-gray-200" style={{ backgroundColor: currentColor }} />
      ...
      {/* 桌面端：跟随位置的小弹窗 */}
      {textInput && (
        <div className="hidden md:block absolute z-20 animate-scale-in" style={{ left: textInput.screenX, top: textInput.screenY }}>
          <div className="bg-white rounded-xl shadow-2xl border border-gray-200 overflow-hidden">
            <div className="p-3">
              <textarea
                ...
                style={{ color: currentColor, fontSize: Math.max(currentLineWidth * 2, 14) }}
              />
            </div>
            <div className="px-3 pb-3 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <span className="w-4 h-4 rounded-full border border-gray-200" style={{ backgroundColor: currentColor }} />
                <span className="text-xs text-gray-400">当前颜色</span>
              </div>
              <div className="flex items-center gap-1.5">
                <button onClick={handleTextCancel} className="px-3 py-1.5 text-xs text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded-lg transition-colors">取消</button>
                <button onClick={handleTextSubmit} disabled={!textValue.trim()} className="px-3 py-1.5 text-xs text-white bg-sky-500 hover:bg-sky-600 disabled:bg-gray-300 disabled:cursor-not-allowed rounded-lg transition-colors">确定</button>
              </div>
            </div>
            <div className="px-3 py-2 bg-gray-50 border-t border-gray-100">
              <p className="text-xs text-gray-400">Enter 确认 · Shift+Enter 换行 · Esc 取消</p>
            </div>
          </div>
        </div>
      )}
