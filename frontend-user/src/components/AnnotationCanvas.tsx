  ...
  switch (annotation.type) {
    ...
    case "polygon":
      if (scaledPoints.length >= 2) {
        ...
        // 如果是闭合多边形（至少3个点且最后一个点接近第一个点），则闭合
      ...
      if (selectedId === a.id && !isHoveredForErase) {
        ...
        if (a.type === "polygon" && a.points.length >= 3) {
          ...
          scaledPoints.forEach((p) => {
            ctx.beginPath();
            ctx.arc(p.x, p.y, cornerSize, 0, Math.PI * 2);
            ctx.fill();
          });
          // 绘制多边形边框
          ctx.setLineDash([4, 3]);
  ...
  const handlePointerDown = useCallback(
    (e: React.PointerEvent<HTMLCanvasElement>) => {
      ...
      const isDoubleClick =
        ...
        Math.abs(lastClickPointRef.current.x - basePoint.x) < 5 &&
        Math.abs(lastClickPointRef.current.y - basePoint.y) < 5;

      lastClickTimeRef.current = now;
      lastClickPointRef.current = basePoint;
